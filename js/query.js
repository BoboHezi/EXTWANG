...
var query = (listUid) => {
    ...
    sendMessage('{type: "fake_ajax", conf:' + JSON.stringify(conf) + '}', function(response) {
        if (response && response.type == 'success') {
            parseDom(response);
            sendMessage({
                type: 'log',
                log: 'fake response: ' + response
            }, null);
        }
    });
}
